...
locals {
  ...
  palo_tag = var.palo_proxy_enabled == true && var.network_project_override == "pcln-pl-net-corp" ? module.gcp_varlib.paloproxy_region_network_tag["corp"][var.region] : module.gcp_varlib.paloproxy_region_network_tag[var.env][var.region]
...
resource "google_compute_disk" "data" {
  count = ((var.data_disk_size != null) || (length(var.data_disk_size_map) > 0)) ? var.module_count : 0

  name = "${local.name_prefix}${format("%02d", count.index + var.sequence_to_start_with)}-${format("sd%s", module.gcp_varlib.alphabet[1])}"
  type = var.disk_type
  size = (length(var.data_disk_size_map) > 0 ? var.data_disk_size_map[count.index] : var.data_disk_size)
